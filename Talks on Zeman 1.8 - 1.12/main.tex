...
\begin{lemma} \label{lemma:definability in very good parameters}
  If $R^{n}_{M} \neq \emptyset$, then
  $\boldsymbol{\Sigma}_{l}(M) \subseteq
  \boldsymbol{\Sigma}^{(n)}_{l}(M)$ for every $l \ge 1$.
\end{lemma}
...
\begin{exercise}
  ...
  % \begin{exercise}
  %   Determine $x_{0}, x_{1}, x_{2} \in [\ord]^{<\omega}$ such that
  %   \begin{enumerate}
  %   \item $\otp (\textvisiblespace <^{*} x_{0} ) = 5$,
  %   \item $\otp (\textvisiblespace <^{*} x_{1} ) = \omega$,
  %   \item $\otp (\textvisiblespace <^{*} x_{2} ) = \omega + \omega$.
  %   \item Suppose $\kappa$ is a cardinal in $L$. Show that
  %     \[
  %       \otp (\textvisiblespace <^{*} \{ \kappa \} ) = \kappa.
  %     \]
  %   \end{enumerate}
  % \end{exercise}
